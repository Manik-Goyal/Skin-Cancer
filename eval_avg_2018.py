...
parser = argparse.ArgumentParser(prog='eval_avg_2018.py', description='evaluates the average ensemble.')
parser.add_argument('--predictions_file', type=str, help='input file with the test data predictions')
...
image_ids, labels, features = su.read_pickled_data(FLAGS.predictions_file)
...
predictions_mel = probability_from_logits(features[:,0])
predictions_nv = probability_from_logits(features[:,1])
predictions_bcc = probability_from_logits(features[:,2])
predictions_akiec = probability_from_logits(features[:,3])
predictions_bkl = probability_from_logits(features[:,4])
predictions_df = probability_from_logits(features[:,5])
predictions_vasc = probability_from_logits(features[:,6])
...
if FLAGS.pool_by_id=='none' :
  ...
  def print_result() :
    if FLAGS.pool_by_id=='avg' :
      ...
      _all.extend(np.column_stack( (extreme_probability(all_mel), extreme_probability(all_nv), extreme_probability(all_bcc), extreme_probability(all_akiec), extreme_probability(all_bkl), extreme_probability(all_df), extreme_probability(all_vasc)) ) )
