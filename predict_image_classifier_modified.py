...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    if FLAGS.extract_features :
      ...
      def print_replica(image_id, label, feats, r) :
        output_file=FLAGS.output_file+str(r)
      ...
      def print_replica(image_id, label, scores, pred, r) :
        output_file=FLAGS.output_file+str(r)
        outfile = open(output_file, 'a')   
        record  = [ image_id ] if FLAGS.id_field_name else  [ ]
        record += [ str(label) ]
      ...
      for s in range(num_samples) :
        ...
        if FLAGS.extract_features :
          ...
          if pooled_features :
            ...
            for r in range(FLAGS.eval_replicas) :
              print_replica(next_id, next_lab, next_feats[r], r)
        else :
          if pooled_scores :
            ...
            for r in range(FLAGS.eval_replicas) :
              print_replica(next_id, next_lab, next_scores[r], next_preds[r], r)
  ...
  if pooled_scores and not FLAGS.extract_features :
    ...
    try :
      ...
      for j in range(num_classes) :
        ...
        np_pred_j   = np.int64(np_predictions ==j)
        
        
        acc_j       = np.mean(np.equal(np_pred_j, np_labels_j))
        accs.append(acc_j)
        
        b_acc_j     = np.sum( np.equal(np_pred_j, 1)& np.equal(np_labels_j,1) )/np.sum(np_labels_j)
        baccs.append(b_acc_j)

        print('Acc[%d]: ' %j, acc_j)
        print('Bal_Acc[%d]: '%j, b_acc_j)
        ...
        mAPs.append(mAP)
        print('mAP[%d]: ' % j, mAP)
        print('Confusion Matrix:\n', sklearn.metrics.confusion_matrix(np_labels_j, np_pred_j))
      
      print("Categorical Acc: ", sum(accs) / num_classes)
      print("Balanced Acc: ", sum(baccs) / num_classes)
